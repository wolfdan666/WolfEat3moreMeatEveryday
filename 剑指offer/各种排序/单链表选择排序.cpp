// 代码借鉴自   https://blog.csdn.net/liming0931/article/details/7726590
...
// 这种实现方法仿空间复杂度O(1)的数组选择排序算法写成,因而也是不稳定的
LinkList SelectSort2(LinkList L){
    ...
    for(p = L->next; p->next != NULL; p = p->next){
        ...
        // 对哦,链表选择排序的灵魂就在这里，就是说只交换value就行了
        if(small != p){
            temp = p->data;
            p->data = small->data;
            small->data = temp;
...
// 这种是稳定的,因为这是拆解,然后重组---等价于数组选择排序中利用了O(n)的辅助数组一般
