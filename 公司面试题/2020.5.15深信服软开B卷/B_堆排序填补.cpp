/*
2020年5月16日16:56:47 --- 然后玩去了
2020年5月16日20:48:39 晚上才AC
*/
...
static void heap_arrange(int arr[], int cur, int cnt)  //调整为小顶堆
{
    ...
    while (cur < cnt) {
        ...
        int right = 2 * cur + 2;
        int min = -1;
        // int min_val = arr[left] < arr[right] ? arr[left] : arr[right];
        ...
        if (min == -1)
            break;
        // 本尊被min覆盖，本尊要继续向下比较，但是arr[min]没变，所以上面
        // min_val 应该 = heaptop_val; 最后arr[cur] = heaptop_val;在子孙处填入heaptop_val
        arr[cur] = arr[min];
        cur = min;
    }
    arr[cur] = heaptop_val;
