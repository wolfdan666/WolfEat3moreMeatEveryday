...
/*
2019年10月07日21:36:52 又学了一遍AC自动机再来看这个
*/
...
void dfs(int v, int cur){
    // 递归和解一些题目中常用地处理掉之后再回溯的操作
    upd(cur, 1);
    for (auto it : nms[v])
        for (auto q : reqs[it])
            ans[q] = sum(qr[q]);
    forn(i, AL) if (trienm[v][i] != -1)
    // 2019年10月07日22:04:27 还是没有太看懂,今天不早了,明天来看吧,要珍爱身体,珍爱生活,odk
...
int main(){
    ...
    // 2019年10月07日21:36:24 确实是要对模式串建立自动机的
    cntqr = 0;
    trieqr[cntqr++] = node_at();
    ...
    // 询问串中的节点个数cntqr
    // 从最长后缀放入自己,就是说后缀扩展前面的长度,这里是一个后缀树的反向链
    for (int v = 1; v < cntqr; ++v)
        g[get_link(v)].push_back(v);
    
    T = 0;
    // dfs_init是把断断续续的后缀树反向链连接起来
    dfs_init(0);
    dfs(0, 0);
